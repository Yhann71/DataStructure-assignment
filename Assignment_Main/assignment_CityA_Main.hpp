...
node* readCSV(const string& filename)
{
    ...
    while (getline(file, line))
    {
        ...
        getline(ss, token, ','); newNode->data.Age = stoi(token);
        getline(ss, newNode->data.ModeOfTransport,   ',');
        getline(ss, token, ','); newNode->data.DailyDistance = stoi(token);
        getline(ss, token, ','); newNode->data.CarbonEmissionFactor = stod(token);
        getline(ss, token, ','); newNode->data.AverageDaysPerMonth = stoi(token);
...
// Forward declaration — binarySearch is defined later in Search Algorithms section
